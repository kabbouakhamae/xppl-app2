<template>
  ...
  <div v-else>
      ...
      <div class="card-header pd-r-15 pd-t-10 pb-0">
        <div class="d-flex justify-content-between">
          <div class="d-flex justify-content-start align-items-center">
            ...
            >Roster From {{date1(datefr)}} To {{date1(dateto)}}</h4>
          </div>
          <div class="d-flex justify-content-start">
            ...
            <div class="dropdown-menu rounded-5 box-shadow-pink tx-13">
              ...
              <div class="dropdown-item cur-pointer dropdown-hover" @click="breakInfo()">
                <i class="mdi mdi-airplane-takeoff tx-15 me-2"></i>
                <span>Roster Break Info</span>
              </div>
              <div class="dropdown-item cur-pointer dropdown-hover" @click="shiftWork()">
                <i class="mdi mdi-brightness-6 tx-15 me-2"></i>
                <span>Shift Work Update</span>
              </div>
              <div class="dropdown-item cur-pointer dropdown-hover" @click="addDelete()">
                <i class="mdi mdi-contrast-box me-2 tx-16"></i>
                <span>Add/Delete Roster</span>
      ...
      <div class="card-body pd-t-0 pd-b-0">
          ...
          <div class="d-md-flex justify-content-end mt-xl-0 mt-lg-1 mt-md-1 mt-1">
            ...
            <div class="d-flex justify-content-end">
              ...
              <div class="wd-35 ms-1">
                <button
                  type="button"
                  class="btn btn-outline-primary p-0 border wd-35"
                  title="Reload all"
                  @click="getRoster()"
                >
        ...
        <div class="table-responsive border" style="max-height: 77vh">
          <table class="table main-table-reference text-nowrap mg-b-0">
              ...
              <tr>
                ...
                <th class="px-1 position-sticky border-start-0" style="left: -1px">
                  <div class="d-flex justify-content-between">
                    Name and Surname
                    <i
                      v-if="nameFilter == 'true'"
                      class="mdi mdi-filter-remove tx-10 ms-1 cur-pointer"
                      title="Filter"
                      @click="rosFilterNames()"
                    ></i>
                    <i
                      v-else
                      class="mdi mdi-filter tx-10 ms-1 cur-pointer"
                      title="Filter"
                      @click="rosFilterNames()"
                    ></i>
                  </div>
                ...
                <th v-if="showCol.position" class="px-1 border-start-0">
                  <div class="d-flex justify-content-between">
                    Position
                    <i
                      v-if="filterMode == 'Position'"
                      class="mdi mdi-filter-remove tx-10 ms-1 cur-pointer"
                      title="Filter"
                      @click="FilterByPosition()"
                    ></i>
                    <i
                      v-else
                      class="mdi mdi-filter tx-10 ms-1 cur-pointer"
            ...
            <tbody>
              <tr
                v-for="(row, rowInx) in rosData"
                :key="rowInx"
                @click="selectRow(row.userid, rowInx, row.fullname, row.rtype)"
                :style="rowInx === rowSel && row.rtype == 'P' ? 'color: red; font-weight: 500' : rowInx === rowSel && row.rtype == 'A' ? 'color: blue; font-weight: 500' : ''"
              >
                ...
                <td
                  v-if="showCol.position"
                  class="px-1"
                  :title="row.position"
                >{{cutWord(row.position)}}</td>
                <td
                  v-if="showCol.crew"
                  class="px-1 border-start-0 cur-pointer"
                  @dblclick="newCrew(row.id)"
                >{{row.crew}}</td>
                <td
                  v-if="showCol.section"
                  class="px-1 cur-pointer"
                  @dblclick="newSection(row.id)"
                >{{row.section}}</td>
                ...
                <td
                  v-for="(col, colInx) in colData"
                  :key="colInx"
                  class="px-1 text-center align-middle"
                  style="font-size: 12px"
                  :title="comm(row[col.colid]) ? mentDate(col.colid) +'  '+ comm(row[col.colid]) : mentDate(col.colid)"
                ...
                >
                  <div
                    v-if="code(row[col.colid])"
                    class="cur-pointer"
                    @dblclick="UpdRoster_Range(col.colid, code(row[col.colid]), comments(row[col.colid]), colInx), optMode='Update'"
                  >
                    <div
                      v-if="comm(row[col.colid])"
                      class="text-decoration-underline"
                    >{{code(row[col.colid])}}</div>
                    <div v-else>{{code(row[col.colid])}}</div>
                  </div>
                  <div v-else class="p-0 cur-pointer">
                    <button
                      ...
                      title="Add"
                      @dblclick="UpdRoster_Range(col.colid, code(row[col.colid]), comments(row[col.colid]), colInx), optMode='Add'"
                    ></button>
                  </div>
                </td>
              </tr>
        ...
        <div class="d-flex justify-content-between tx-13 pt-1 pd-b-6">
          ...
          <div v-if="text" class="text-end me-1">
            {{text}}
            <span class="fw-bold text-primary">{{count}}</span> days
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="modal-content">
          <div class="modal-header pb-1 bd-b-0">
            <h6 class="main-content-label text-capitalize">
              {{shortname(fullname)}}:
              <span
                v-if="rosForm.rtype=='P'"
                class="tx-danger"
              >{{type(rosForm.rtype)}}</span>
          ...
          <div class="modal-body pt-1">
            ...
            <div class="d-flex justify-content-between">
              <div v-if="optMode !='Add'">
                <label class="rdiobox cur-pointer my-0">
                  <input name="opt1" type="radio" v-model="optMode" value="Update" checked />
                  <span class="ps-2">Update</span>
                </label>
                <label class="rdiobox cur-pointer my-0">
                  <input name="opt1" type="radio" v-model="optMode" value="Delete" />
                  <span class="ps-2">Delete</span>
                </label>
              </div>
              <div v-else class="d-flex justify-content-center align-items-center">
                <label class="rdiobox cur-pointer my-0">
                  <input name="opt1" type="radio" v-model="optMode" value="Add" checked />
              ...
              <div class="d-flex justify-content-end">
                ...
                <button
                  v-else-if="optMode=='Delete'"
                  type="button"
                  class="btn btn-danger"
                  :class="DelRosRangeDis"
                  @click="DelRoster_RangeCF()"
                >
        ...
        <div class="modal-content">
          <div class="modal-header pb-1 bd-b-0">
            ...
            <div>
              <button
                type="button"
                class="btn btn-sm btn-secondary r-left"
                style="width: 55px"
                title="Roster details"
                @click="detailMode = 'roster'"
              >Roster</button>
              <button
                type="button"
                class="btn btn-sm btn-secondary rounded-0"
                style="width: 55px"
                title="Finger scan details"
                @click="detailMode = 'scan'"
              >Scan</button>
              <button
                type="button"
                class="btn btn-sm btn-secondary r-right"
          ...
          <div class="modal-body pt-0">
            <div class="table-responsive border" style="max-height: 700px">
              <table
                v-if="detailMode == 'roster'"
                class="table main-table-reference text-nowrap mg-b-0"
              >
                ...
                <tbody>
                  <tr class="tr-hover" v-for="lst in detailData" :key="lst.rdate">
                    <td
                      class="border-start-0 py-0 text-center border-bottom-0"
                    >{{ date1(lst.rdate) }}</td>
                    <td
                      :style="lst.pcode == 'W' || lst.pcode == 'WN' ? 'background-color: #F2F4F8':
                                                    ...
                                                    lst.pcode == 'T' || lst.pcode == 'T-' ? 'background-color: #FF0000':''"
                      class="py-0 text-center border-bottom-0"
                    >{{ lst.pcode }}</td>
                    <td
                      :style="lst.acode == 'W' || lst.acode == 'WN' ? 'background-color: #F2F4F8':
                                                    ...
                                                    lst.acode == 'T' || lst.acode == 'T-' || lst.acode == 'IT' || lst.acode == 'OT' || lst.acode == 'TT' ? 'background-color: #FF0000':''"
                      class="py-0 text-center border-bottom-0"
                    >{{ lst.acode }}</td>
              ...
              >
                ...
                <tbody>
                  <tr class="tr-hover" v-for="lst in scanData" :key="lst.date">
                    ...
                    <td
                      :style="lst.timesheet == 'W' || lst.timesheet == 'WN' ? 'background-color: #F2F4F8':
            ...
            <div class="row">
                        ...
                        <th class="border-0" style="padding: 5px 10px">
                          <label class="form-checkbox m-0">
                            <div class="d-flex justify-content-between align-items-center">
                              <input
                                type="checkbox"
                                v-model="add_selectAll"
                                @click="AddSelectAll_click()"
                              />
                              <i class="form-icon"></i>
                              <span
                                class="ms-2 cur-pointer mg-t-2"
              ...
              <div class="col-lg-5">
                <div class="d-flex justify-content-start mt-md-2 mt-4">
                  <label class="rdiobox cur-pointer" @click="addMethodChanged()">
                    <input name="add" type="radio" value="add" checked v-model="addMethod" />
                    <span>Add</span>
                  </label>
                  <label class="rdiobox cur-pointer ms-5" @click="addMethodChanged()">
                    <input name="add" type="radio" value="delete" v-model="addMethod" />
                    <span>Delete</span>
                  </label>
                </div>
                <div class="form-group mt-3">
                  <label v-if="addMethod == 'add'" class="mb-0">
                    Add Until Date
        ...
        <div class="modal-content">
          <div class="modal-header pb-1 bd-b-0">
            ...
            <button
              ...
              style="padding: 3.2px 11.2px height: 15px; width: 70px"
                ...
                <tbody>
                  <tr v-for="lst in rosCodeData" :key="lst.id">
                    ...
                    <td v-if="permiss.lk_ros == 1" class="p-0 align-middle">
                      <div class="d-flex justify-content-start">
                        <button
                          class="btn btn-icon btn-sm btn-i p-0"
                          title="Edit this record"
                          @click="rosCodeEdit(lst.id)"
                        >
    ...
    <div
      class="modal fade effect-scale pd-t-100 bd-0 bg-black-5"
      id="rosCodeEdit"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="rosCodeLabel"
      aria-hidden="true"
    >
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="d-flex justify-content-end">
              ...
              <button v-else type="button" class="btn btn-primary" @click="rosCodeUpd()">
    ...
    <!-- Modal Show Columns -->
    <div
      class="modal fade effect-scale bd-0"
      id="showCol"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="colShowLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body pt-0">
            <div class="d-flex justify-content-between mb-2">
              ...
              <div class="custom-control custom-switch">
                ...
                />
                <label
                  class="custom-control-label cur-pointer"
                  for="no"
                ></label>
              </div>
            </div>
            ...
            <div class="form-group">
              <div class="d-flex justify-content-between">
                <div class="wd-100p me-1">
                  <Multiselect
                    v-model="crewForm.crew"
                    :searchable="true"
                    :options="lkCrew"
                    @keydown.enter="addCrew()"
                  />
                </div>
                <div class="wd-35">
                  <button
                    type="button"
                    class="btn btn-primary wd-35 p-0"
                    :class="addCrewDis"
                    title="Add new group"
                    @click="addCrew()"
                  >
    ...
    <div
      ...
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="shiftWorkLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content">
    ...
    <div
      ...
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="searchNameLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body pt-2">
            ...
            <div class="d-flex justify-content-end mt-2">
              <button
                type="button"
                class="btn btn-primary"
                :class="rosFilterNamesDis"
                @click="filterNames(), nameFilter ='true'"
              >
                <span class="px-3">OK</span>
              </button>
    ...
    >
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header pb-1 bd-b-0">
            <h6
              class="main-content-label text-capitalize"
            >ລາຍຊື່ພະນັກງານທີ່ໃກ້ຈະອອກເມື່ອພັກອີກພາຍໃນ 5 ວັນຂ້າງໜ້າ</h6>
            <button aria-label="Close" class="close" data-bs-dismiss="modal" type="button">
              <span class="tx-24" aria-hidden="true">×</span>
            </button>
          </div>
          <div class="modal-body pt-0">
            ...
            <p
              class="laofont mt-2 mb-0 font-italic tx-danger"
            >ໝາຍເຫດ: ຫຼັງຈາກຂຽນໃບລາພັັກແລ້ວກະລຸນາອັບເດດໂຣສເຕີ</p>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    async onLoad() {
      ...
      const colhead = await axios.get(
        `/api/roster/datecol?datefr=${this.datefr}&dateto=${this.dateto}`
      );
      this.colData = colhead.data;

      const roster = await axios.get(
        `/api/roster/roster?dept=${this.dept}&datefr=${this.datefr}&dateto=${this.dateto}&search=${this.search}`
      );
    ...
    async getRoster() {
      ...
      if (this.dept != "" && this.datefr != "" && this.dateto != "") {
        const resCol = await axios.get(
          `/api/roster/datecol?datefr=${this.datefr}&dateto=${this.dateto}`
        );
        this.colData = resCol.data;

        const resRos = await axios.get(
          `/api/roster/roster?dept=${this.dept}&datefr=${this.datefr}&dateto=${this.dateto}&search=${this.search}`
        );
    ...
    UpdRoster_RangeCF() {
      ...
      for (let i = 0; i < this.rosForm.days; i++) {
        ...
        this.rosData.find(
          i => i.userid == this.rosForm.userid && i.rtype == this.rosForm.rtype
        )[col] = this.rosForm.rcode;
        this.rosData.find(
          i => i.userid == this.rosForm.userid && i.rtype == this.rosForm.rtype
        )[col] = this.rosForm.rcode;
      ...
      this.$axios
        .post("/api/roster/updrosterrange", {
          list: dateList,
    ...
    DelRoster_RangeCF() {
      ...
      this.$swal
        .fire({
          ...
          confirmButtonText:
            '<i class="fe fe-trash-2"></i> <span class="font-weight-light">Delete</span>',
          cancelButtonText:
            '<i class="fe fe-x"></i> <span class="font-weight-light">Cancel</span>',
        ...
        .then(result => {
          if (result.isConfirmed) {
            ...
            for (let i = 0; i < this.rosForm.days; i++) {
              let col = this.colData[n].colid;
              this.rosData.find(
                i => i.userid == this.rosForm.userid && i.rtype == "P"
              )[col] = null;
              this.rosData.find(
                i => i.userid == this.rosForm.userid && i.rtype == "A"
              )[col] = null;
            ...
            //delete in database
            this.$axios
              .post("/api/roster/delrosterrange", {
                list: dateList,
                userid: this.rosForm.userid
              })
              .then(res => {
    ...
    AddRoster_RangeCF() {
      ...
      this.$axios
        .post("/api/roster/addrosterrange", {
          list: dateList,
          userid: this.rosForm.userid,
          code: this.rosForm.rcode
        })
    ...
    FilterList() {
      ...
      this.$axios
        .get(`/api/roster/filterlist?dept=${this.dept}&filter=${this.filterBy}`)
    ...
    async FilterResult() {
      ...
      if (this.dept != "" && this.datefr != "" && this.dateto != "") {
        ...
        const resRos = await axios.post("/api/roster/filterresult", fd, {
    ...
    async filterNames() {
      ...
      if (this.dept != "" && this.datefr != "" && this.dateto != "") {
        ...
        const resRos = await axios.post("/api/roster/rostermultiname", fd, {
          headers: { "Content-Type": "multipart/form-date" }
        });
        this.rosData = resRos.data;
    ...
    rosCodeAdd() {
      this.$axios
        .post("/api/roster/rosCodeAdd", this.rosCodeForm)
        .then(res => {
          if (res.data.success) {
    ...
    rosCodeEdit(id) {
      ...
      this.$axios
        .post(`/api/roster/rosCodeEdit/${id}`)
        .then(res => {
    ...
    rosCodeUpd() {
      this.$axios
        .post("/api/roster/rosCodeUpd", this.rosCodeForm)
        .then(res => {
          $("#rosCodeEdit").modal("hide");
          this.getRosCode();
        })
    ...
    countDay(inx, value) {
      ...
      if (
        ...
        d1 != "" &&
        d2 != "" &&
        moment(d2).format("YYYYMMDD") > moment(d1).format("YYYYMMDD")
      ) {
    ...
    addCrew() {
      this.$axios.post("/api/roster/addcrew", this.crewForm).then(res => {
        $("#addCrew").modal("hide");
        // this.getRoster();
      });
    },
    ...
    addSection() {
      this.$axios.post("/api/roster/addsection", this.sectForm).then(res => {
    ...
    async nameList() {
      const names = await axios.get(
        `/api/lookup/emplist?dept=${this.dept}&search=${this.search2}`
      );
    ...
    breakInfo() {
      this.$axios.get(`/api/roster/breakinfo?dept=${this.dept}`).then(res => {
