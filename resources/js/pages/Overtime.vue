      ...
      <div class="card-header pd-r-15 pd-t-10 pb-0">
        <div class="d-flex justify-content-between">
          <div class="d-flex justify-content-start align-items-center">
            <h4 class="card-title mg-b-0 text-muted">Overtime Records {{testVal}}</h4>
          </div>
          <div class="d-flex justify-content-start">
            ...
            <div class="dropdown-menu rounded-5 box-shadow-pink tx-13">
              ...
              <div class="dropdown-item cur-pointer dropdown-hover" @click="RunUpdate_Scan()">
                <i class="far fa-hand-pointer me-2 tx-16"></i>
                <span>Run OT Fingerscan</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="card-body pd-t-0 pd-b-0">
        <div class="d-xl-flex justify-content-between mt-1 mb-1">
          <div class="pos-relative wd-lg-300 wd-100p">
            <input
              ...
              v-model="search"
              @input="searchChange()"
              @keyup.enter="getOvertime()"
              title="Search by name, Crew or Section"
            />
        ...
        <div class="table-responsive border" style="max-height: 77vh">
          <table class="table main-table-reference text-nowrap mg-b-0">
            ...
            <tbody>
              <tr
                v-for="(row, rowInx) in otData"
                :key="rowInx"
                @click="selectRow(row.userid, rowInx, row.fullname)"
                :style="rowInx === rowSel ? 'color: blue; font-weight: 500' : ''"
              >
                <td class="px-1 text-center border-start-0">{{rowInx + 1}}</td>
                <td
                  class="px-1 position-sticky cur-pointer border-start-0 bg-white"
                  style="left: -1px"
                  title="Click to see OT details"
                  @click="OTDetail(row.userid)"
                >{{row.fullname}}</td>
                ...
                <td
                  ...
                  :title="code(row[col.colid]) ? mentDate(col.colid) +'  ('+ comm(row[col.colid]) +') '+ scan(row[col.colid]):''"
                  :style="paid(row[col.colid])=='Y' ? 'background-color: #C8E6C9': 
                                            ...
                                            scan(row[col.colid])=='Missed check out' || scan(row[col.colid])=='Early check out' || scan(row[col.colid])=='Late check in' || scan(row[col.colid])=='Missed check in' ? 'background-color: #FF5722':''"
                  @click="countDays(rowInx, headDate1(col.colid), colInx)"
                >
                  <div
                    v-if="row[col.colid]"
                    class="cur-pointer"
                    @dblclick="UpdOT_Range(col.colid, colInx), optMode='Update'"
                  ...
                  <div v-else class="p-0">
                    <button
                      ...
                      title="Add"
                      @dblclick="AddOT_Range(col.colid, colInx), optMode='Add'"
                    ></button>
                  </div>
                </td>
              </tr>
        ...
        <div class="d-flex justify-content-between tx-13 pt-1 pd-b-6">
          <div class="d-flex justify-content-start">
            ...
            <span
              class="tx-11 px-1 pd-t-1 border ms-1"
          ...
          <div v-if="countTxt" class="text-end me-1">
            {{countTxt}}
            <span class="fw-bold text-primary">{{countNum}}</span>
          </div>
        </div>
      </div>
              ...
              <div class="row">
                ...
                <div class="col-lg-5">
                  <div class="d-flex justify-content-start mt-md-2 mt-4">
                    <label class="rdiobox cur-pointer">
                      <input name="add" type="radio" value="add" checked v-model="opt" />
                      <span class="ps-2">Add</span>
                    </label>
                    <label class="rdiobox cur-pointer ms-5">
                      <input name="add" type="radio" value="delete" v-model="opt" />
                      <span class="ps-2">Delete</span>
                    </label>
                  </div>
                  ...
                  <tbody>
                    <tr class="tr-hover" v-for="lst in otDetail" :key="lst.id">
                      <td
                        ...
                        @click="PrvAcLog(lst.userid, lst.ot_date)"
                        title="Click to see AcLog"
                      >{{date1(lst.ot_date)}}</td>
                ...
                <table class="table main-table-reference text-nowrap mg-b-0">
                  ...
                  <tbody>
                    <tr class="tr-hover" v-for="(lst, inx) in aclogFilter" :key="inx">
                      ...
                      <td class="py-0 text-center border-bottom-0">{{date3(lst.check_time)}}</td>
                      <td class="py-0 text-center border-bottom-0">{{date1(lst.scan_date)}}</td>
...
export default {
  ...
  data() {
    return {
      ...
      lkType: [
        { value: "S", label: "ກ່ອນເລີ່ມຜຽນວຽກ" },
        { value: "E", label: "ຫຼັງຈາກຜຽນວຽກ" },
        { value: "D", label: "ທ່ຽງວັນ" },
        { value: "N", label: "ທ່ຽງຄືນ" }
      ],
  ...
  computed: {
    addOTDis() {
      if (
        ...
        this.otForm.datefr == "" ||
        this.otForm.dateto == "" ||
        this.otForm.time_start == "" ||
        this.otForm.time_end == "" ||
        this.otForm.ot_type == null ||
        this.otForm.datefr > this.otForm.dateto
      ) {
        return "disabled";
      } else {
        return "";
  ...
  methods: {
    async onLoad() {
      ...
      const data = await this.getOvertime();
      this.loading = false;
    ...
    async getOvertime() {
      ...
      if (this.dept != "" && this.datefr != "" && this.dateto != "") {
        ...
        const ot = await axios.get(
          `/api/overtime/otsearch?dept=${this.dept}&datefr=${this.datefr}&dateto=${this.dateto}&search=${this.search}`
        );
        this.otData = ot.data;
    ...
    async UpdOT_Range(dt, inx) {
      ...
      const otEdit = await axios.get(
        `/api/overtime/editot?userid=${this.otForm.userid}&date=${date}`
      );
    ...
    RunUpdate_Scan() {
      this.$axios.post("/api/overtime/runscan").then(res => {
        this.getOvertime();
      });
    },
    ...
    PrvAcLog(id, date) {
      ...
      // console.log(this.aclogData);
      // console.log(this.otDetail);
      console.log(id);
      console.log(date);
